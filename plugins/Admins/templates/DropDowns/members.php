   ...
   <div id="page-inner">
      <div class="row">
         <div class="col-md-12">
            ...
            <h4>Total Team Members: <?php echo $this->Paginator->counter('{{count}}'); ?></h4>
         </div>
      </div>
      <!-- /. ROW  -->
      <hr />
               ...
               <div class="panel-heading" style="padding: 10px 15px 10px;">
                  <div class="row">
                     <div id="searchDivOptions" style="display:block;">
                        ...
                        <div class="col-md-2">
                           <?php $phone = $this->request->getQuery('phone'); ?>
                           <?php echo $this->Form->control('phone',array(
                              ...
                              'required'=>false,
                              'class'=>'form-control',
                              'value' => isset($phone)?$this->request->getQuery('phone'):''
                              )
                            );
                           ?>
                        </div>
                        ...
                        <?php
                           echo $this->Html->link('Reset',
                           	array('plugin' => 'Admins', 'controller' => 'DropDowns', 'action' => 'members'),
                           	array('escape' => false,
                           		'title'=>'Click here to reset record','class' => "btn btn-warning",'type'=>"button")
                           );
                        ?>
                        <?php
                              echo $this->Html->link('Add Member',
                              array('plugin' => 'Admins', 'controller' => 'DropDowns', 'action' => 'addMember'),
                              array('escape' => false,
                                 'title'=>'Click here to add member','class' => "btn btn-default",'type'=>"button", "style"=>'float:right;margin:0px 15px 0px 0px;')
                           );
                        ?>
                     ...
                     <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                        <thead>
                           <tr>
                              ...
                              <th><?= $this->Paginator->sort('Profile', 'Profile') ?></th>
                        ...
                        <tbody>
                           <?php if(isset($members) && $members->count() > 0){
						   	$num = 0; 
						   ?>
                           ...
                           <tr class="odd gradeX">
                              ...
                              <td><?php echo ucwords(isCheckVal($val->email)); ?></td>
                              <td><?php echo ucwords(isCheckVal($val->phone)); ?></td>
                              <td align="center">
                                 <?php 
                                 if($val['profile'] != ''){
                              ...
                              <td align="center">
                                 <?php if($val->status == 1){ ?>                                  
                                 <button id="buttonId_<?php echo $val->id; ?>" onclick="changeStatus('Members',<?php echo $val->id; ?>,<?php echo $val->status; ?>)" title= "Click here to Deactive" class="btn btn-info btn-circle"><i class="fa fa-check"></i></button>
                                 <?php } else { ?>
                                 <button id="buttonId_<?php echo $val->id; ?>" onclick="changeStatus('Members',<?php echo $val->id; ?>,<?php echo $val->status; ?>)" title= "Click here to Active" class="btn btn-warning btn-circle"><i class="fa fa-times"></i></button>
                                 <?php } ?>       
                              ...
                              <td>
                                 <?php
                                    echo $this->Html->link('<i class="fa fa-edit "></i> Edit',
                                    	array('plugin' => 'Admins', 'controller' => 'DropDowns', 'action' => 'addMember', base64_encode($val->id)),
                                    	array('escape' => false,
                                 ...
                                 ?>
                                 <button class="btn btn-danger" onclick="deleteRecord('Members',<?php echo $val->id; ?>,0);" ><i class="fa fa-pencil"></i> Delete</button>
                              </td>
                           </tr>
                           <?php $num++; endforeach;  ?>
                           <?php }else{ ?>
...
<script type="text/javascript">
   function searchData(){
      window.location.href="<?php echo $this->Url->build('/admins/drop-downs/members/'); ?>"; 
   }
</script>